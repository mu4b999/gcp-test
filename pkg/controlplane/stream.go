...
// // OnStreamRequest is called when a DiscoveryRequest is received on an xDS stream.
// func (cp *ControlPlane) OnStreamRequest(id int64, req *discoveryv3.DiscoveryRequest, stream discoveryv3.AggregatedDiscoveryService_StreamAggregatedResourcesServer) error {
// 	nodeID := req.Node.GetId()
// 	if nodeID == "" {
// 		return fmt.Errorf("missing node ID in request")
// 	}
...
// 		state.nodeID = nodeID
// 		state.typeURL = req.TypeUrl
// 		state.stream = stream // Store the gRPC stream for push-based updates
// 	}
// 	cp.streamsMu.Unlock()
